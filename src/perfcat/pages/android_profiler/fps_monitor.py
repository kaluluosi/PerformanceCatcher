...
class FPSMonitorCard(MonitorCard):
    ...
    async def sample(self, serialno: str, app: str, process: str):
        ...
        self._add_point("FPS", fps)
        self._add_point("Jank", stat.jank)
        self._add_point("Big-Jank", stat.big_jank)
        RecordService.logger.info({"name": self.title, "FPS": fps, "Jank": stat.jank, "Big-Jank": stat.big_jank})
        self.update_chart()
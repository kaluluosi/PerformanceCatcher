...
class HomePage(Page):
    ...
    async def render(self):
        ...
        with ui.table(columns=columns,rows=rows).classes("w-full h-full").props("separator=cell bordered striped hoverable") as table:
            ...
            """)
            table.on('action_preview', lambda row: ui.navigate.to(f"/home/report/{row.args['name']}"))
            table.on('action_delete', lambda row: None)
